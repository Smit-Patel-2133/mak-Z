...
const Contact = () => {
  ...
  function contactSubmit(){
    ...
    axios.post('http://localhost:5000/addContacts', {name,email,message})
      .then(() => {
        console.log('Contact Added');
        alert("We will contact you very soon")
      })
      .catch(error => {
        console.error('Error in get addContacts:', error);
      });
    ...
    <div style={{ overflow: 'hidden', height: '100vh' }}>
      ...
      <div style={{ display: 'flex', width: '100vw', height: '100vh', background: '#FCFDFD', overflow: 'hidden' }}>
        ...
        <div style={{ background: '#FCFDFD', width: '50vw', height: '100vh', padding: '5rem 3.5rem', overflow: 'auto' }}>
          ...
          <form id="contact-form" style={{ width: '31.25rem', position: 'relative', marginTop: '2rem', padding: '1rem 0' }}>
            ...
            <button type="submit" id="submit" onClick={contactSubmit} name="submit" style={{ textTransform: 'uppercase', fontWeight: 300, background: '#3B3636', color: '#FCFDFD', width: '10rem', height: '2.25rem', border: 'none', borderRadius: '2px', outline: 'none', cursor: 'pointer' }}>Send</button>
          </form>
